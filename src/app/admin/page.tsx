...
import { PARAS, MODULES, getParaTrackStatus, getUnlockedDuties, getCompletionPct } from '@/lib/demo-data';
import { Progress } from '@/components/ui/progress';
import { Badge } from '@/components/ui/badge';
import { BookOpen, AlertTriangle, CheckCircle, Clock, ChevronLeft, Unlock, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const paras = PARAS.map(p => ({
    ...p,
    status: getParaTrackStatus(p),
    pct: getCompletionPct(p),
    duties: getUnlockedDuties(p),
  }));

  const overdue = paras.filter(p => p.status === 'overdue').length;
  const complete = paras.filter(p => p.status === 'complete').length;
  const inProgress = paras.filter(p => p.status === 'in_progress').length;

  const selectedPara = selected ? paras.find(p => p.id === selected) : null;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <nav className="border-b bg-white px-6 py-3 flex items-center gap-3">
        ...
        <span className="ml-auto text-xs text-slate-400">Riverside Unified School District</span>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: 'Total Paras', value: paras.length, color: 'text-slate-900' },
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          <div className="divide-y">
            {paras.map(para => {
              ...
              const day30Done = MODULES.filter(m => m.day === 30).every(m => para.progress.find(p => p.moduleId === m.id)?.completed);
              const day60Done = MODULES.filter(m => m.day <= 60).every(m => para.progress.find(p => p.moduleId === m.id)?.completed);
              const currentTrack = !day30Done ? '30-day' : !day60Done ? '60-day' : '90-day';
              ...
              return (
                <button key={para.id} onClick={() => setSelected(para.id === selected ? null : para.id)} className="w-full text-left px-5 py-4 hover:bg-slate-50 transition-colors flex items-center gap-4 flex-wrap">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500">{para.role} · Started {new Date(para.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} · {currentTrack} track</div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="hidden sm:flex items-center gap-1.5 text-xs text-emerald-700">
                      <Unlock className="h-3 w-3" />{para.duties.length} duties cleared
                    </span>
        ...
        {selectedPara && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="font-semibold text-slate-900">{selectedPara.name} — Training Detail</h2>
                <p className="text-sm text-slate-500">{selectedPara.role} · {selectedPara.duties.length} duties cleared</p>
              </div>
            ...
            <div className="space-y-2 mb-5">
              {MODULES.map(mod => {
                const prog = selectedPara.progress.find(p => p.moduleId === mod.id);
                const done = prog?.completed ?? false;
                return (
                  <div key={mod.id} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 ${done ? 'bg-emerald-50' : 'bg-slate-50'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Badge variant="outline" className="ml-2 text-xs">{mod.day}-day</Badge>
                    </div>
                    {done ? (
                      <span className="text-xs text-emerald-600 shrink-0">{prog?.completedDate}</span>
                    ) : (
            ...
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Cleared duties ({selectedPara.duties.length})</p>
              <div className="flex flex-wrap gap-2">
                {selectedPara.duties.length > 0
                  ? selectedPara.duties.map(d => <Badge key={d} variant="outline" className="text-xs bg-emerald-50 text-emerald-700 border-emerald-200">{d}</Badge>)
